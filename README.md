...
| `IJAVA_STARTUP_SCRIPTS_PATH` | `startup-scripts-path` | `""` | A file path seperator delimited list of `.jshell` scripts to run on startup. This includes {download}`ijava-jshell-init.jshell <src/main/resources/ijava-jshell-init.jshell>` and {download}`ijava-kernel-metadata.json <src/main/resources/ijava-kernel-metadata.json>`. **Important:** no matter what OS, this should use forward slash "/" as the file separator. Also each path may actually be a [simple glob](#simple-glob-syntax). |
